...
function prompt_zshgod_rectangular_git_info() {
	...
	(( staged_count > 0 )) && parts+=("%F{$fg_color_staged_count}${staged_count}+%f")
	(( modified_count > 0 )) && parts+=("%F{$fg_color_modified_count}${modified_count}*%f")
	(( deleted_count > 0 )) && parts+=("%F{$fg_color_deleted_count}${deleted_count}-%f")
	(( untracked_count > 0 )) && parts+=("%F{$fg_color_untracked_count}${untracked_count}?%f")
	(( ahead_count > 0 )) && parts+=("%F{$fg_color_ahead_count}${ahead_count}↑%f")

	# If there's anything to show, join the parts add do some customization
	(( ${#parts[@]} > 0 )) && print "%K{$bg_color} ${parts[*]} %k%f"
}
